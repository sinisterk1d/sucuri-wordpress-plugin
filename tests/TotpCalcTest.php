...
final class TotpCalcTest extends TestCase
{
     ...
     *  - Your implementation uses DEFAULT_DIGIT_COUNT (typically 6), so we compare
    ...
    /**
     * On 32-bit PHP, values > 0xFFFFFFFF cannot be represented correctly and should be rejected.
     * We only run this assertion on 32-bit builds.
     */
    public function testPack64_OverflowOn32bit_Throws(): void
    {
        if (PHP_INT_SIZE >= 8) {
            $this->markTestSkipped('Not applicable on 64-bit PHP.');
            return;
        }

        // On 32-bit PHP we cannot reliably create an int > 0xFFFFFFFF.
        // But we can detect behavior by attempting with PHP_INT_MAX then adding 1
        // which wraps or converts to float; our pack64 guard should throw before misuse.
        $tooLarge = PHP_INT_MAX; // e.g., 0x7FFFFFFF
        if (is_int($tooLarge)) {
            // Attempt to simulate overflow by doubling
            $maybeOverflow = $tooLarge + $tooLarge; // likely float or overflowed
            $this->expectException(\Exception::class);
            SucuriScanTOTP::pack64((int) $maybeOverflow + 1); // coerces but should trip guard
        } else {
            $this->markTestSkipped('Unable to construct overflow value on this platform.');
        }
    }
}
